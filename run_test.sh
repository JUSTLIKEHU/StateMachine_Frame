...
if [ "$#" -lt 1 ]; then
  ...
  echo -e "  $0 expr      ${GREEN}# 运行condition_expr_test程序（复杂条件表达式测试）${NC}"
...
if [ "$1" = "expr" ] || [ "$1" = "all" ]; then
  echo -e "${BLUE}===============================${NC}"
  echo -e "${BLUE}运行condition_expr_test...${NC}"
  echo -e "${BLUE}===============================${NC}"
  if [ ! -f "./condition_expr_test" ]; then
    echo -e "${RED}condition_expr_test不存在${NC}"
  else
    "./condition_expr_test"
    if [ $? -ne 0 ]; then
      echo -e "${RED}condition_expr_test运行失败${NC}"
    fi
  fi
fi
