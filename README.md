...
- **Original Event on Pending Resume**: When a pending transition is finally consumed by a condition change, the consuming pass typically arrives via the synthetic `__INTERNAL_EVENT__`. To avoid confusing user callbacks, the framework saves the original user event at pending creation and replays it through the resume callbacks (`OnExitState` / `SetState` / `OnEnterState` / `OnPostEvent`). User code therefore always sees the same user-defined event name in those callbacks, regardless of whether the transition fired immediately or was resumed from a pending state.
...
class ITransitionManager : public IComponent {
 public:
  ...
  // Get the user event captured at pending creation. Returned to callers
  // during the resume phase so callbacks always see the original user event
  // instead of an internal/condition-derived event. Returns nullptr if no
...
11. **Pending Transition**
  ...
  struct PendingTransition {
    ...
    EventPtr originalEvent;                            // The user event captured at pending creation (replayed on resume)
  };
  ```
  ...
  - `originalEvent` lets the framework restore the user-facing event when a pending is consumed via a synthetic internal event (e.g. one generated by a condition change). Resume-path callbacks (`OnExitState`/`OnEnterState`/`OnPostEvent`) receive this saved event instead of the internal one.
...
- **Original event on resume**: scenarios that resume a pending also assert that
  `OnPostEvent` reports the user's original event (e.g. `go_heat`, `reset_event`)
  rather than the synthetic `__INTERNAL_EVENT__` that actually triggered the
  consuming pass.
...
When the pending transition is later consumed, the **same original user event** is replayed through `OnExitState` / `SetState` / `OnEnterState` / `OnPostEvent` — even when the actual triggering pass came from a synthetic `__INTERNAL_EVENT__` produced by a condition change. As a result, user callbacks always observe a single, consistent event identity from the early `OnTransition` all the way through to `OnPostEvent`, regardless of whether the rule fired immediately or was deferred. (`OnPreEvent` is intentionally NOT rewritten, because its semantics are "should this incoming event be admitted into processing?" — the actual incoming event must be visible there.)
